using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using MoreMountains.Feedbacks;
using TMPro;
using Nova.TMP;

// 0连击1.0x
// 10连击1.1x
// 20连击1.2x
// 30连击1.3x
// 40连击1.4x
// 50+连击1.5x
public class StarCoin : Singleton<StarCoin>
{
    // [SerializeField] HandnoteChapter chapter;

    [SerializeField] float score;
    [SerializeField] int combo;

    [SerializeField] int maxcombo;
    [SerializeField] int totalNote;

    [SerializeField] int perfectNote;
    [SerializeField] int greatNote;
    [SerializeField] int normalNote;
    [SerializeField] int failNote;


    [SerializeField] TMP_Text scoretext;
    [SerializeField] TMP_Text combotext;


    [SerializeField] TextMeshProTextBlock maxcombotext;
    [SerializeField] TextMeshProTextBlock totalNotetext;
    [SerializeField] TextMeshProTextBlock perfectNotetext;
    [SerializeField] TextMeshProTextBlock greatNotetext;
    [SerializeField] TextMeshProTextBlock normalNotetext;
    [SerializeField] TextMeshProTextBlock failNotetext;
    [SerializeField] TextMeshProTextBlock finalscoretext;


    public MMF_Player addperfectFB;
    public MMF_Player addgreatFB;
    public MMF_Player addnormalFB;
    public MMF_Player failFB;
    public MMF_Player addcomboFB;


     float multiplier=1;
    DetectNotice detectNotice;
    public void LostCap()
    {
        multiplier=0.5f;
    }

    public void NormalCap()
    {
        multiplier=1f;
    }
    private void OnEnable() {
        detectNotice=DetectNotice.Instance;
        detectNotice.lostCap+=LostCap;
        detectNotice.normalCap+=NormalCap;
    }
    private void OnDisable() {
        detectNotice.lostCap-=LostCap;
        detectNotice.normalCap-=NormalCap;
    }




    private void Start()
    {
        score = 0;
        combo = 0;
        maxcombo = 0;
        perfectNote = 0;
        greatNote = 0;
        normalNote = 0;
        failNote = 0;
        totalNote = 0;
    }


    public int AddScore(int addscore)
    {
        ++combo;
        ++totalNote;
        int temp = 0;
        addcomboFB.PlayFeedbacks();
        if (combo >= 50)
        {
            temp = (int)(addscore * 1.5f*multiplier);
        }
        else
        {
            temp=(int)(addscore * (1 + (combo / 10) * 0.1f)*multiplier);
        }
        score += temp;
        return temp;
    }

    public int AddPerfect(int addscore)
    {
        
        ++perfectNote;
        addperfectFB.PlayFeedbacks();
        return AddScore(addscore);
    }

    public int AddGreat(int addscore)
    {
        ++greatNote;
        addgreatFB.PlayFeedbacks();
        return AddScore(addscore);

    }

    public int AddNormal(int addscore)
    {
        ++normalNote;
        addnormalFB.PlayFeedbacks();
        return AddScore(addscore);
    }

    public void FailNote()
    {
        if (maxcombo < combo)
        {
            maxcombo = combo;
        }
        ++failNote;
        ++totalNote;
        combo = 0;
        failFB.PlayFeedbacks();
    }

    public void SetScoreText()
    {
        scoretext.text = score.ToString();
    }

    public void SetComboText()
    {
        combotext.text = combo.ToString();
    }

    public void SetmaxcomboText()
    {
        if (combo > maxcombo)
        {
            maxcombo = combo;
        }
        maxcombotext.text = maxcombo.ToString();
    }

    public void SettotalNoteText()
    {
        totalNotetext.text = totalNote.ToString();
    }
    public void SetPerfectNoteText()
    {
        perfectNotetext.text = perfectNote.ToString();
    }
    public void SetGreatNoteText()
    {
        greatNotetext.text = greatNote.ToString();
    }
    public void SetNormalNoteText()
    {
        normalNotetext.text = normalNote.ToString();
    }

    public void SetfailNoteText()
    {
        failNotetext.text = failNote.ToString();
    }

    public void SetfinalscoreText()
    {
        finalscoretext.text = score.ToString();
    }


    public void EndGame()
    {
        PoseTrack.instance.ClearObserver();
        CountTime.Instance.SetScore((int)score);
        CountTime.Instance.SetRank(GetRank());

        SettotalNoteText();
        SetPerfectNoteText();
        SetGreatNoteText();
        SetNormalNoteText();
        SetfailNoteText();
        SetmaxcomboText();
        SetfinalscoreText();
        CountTime.Instance.EndCountTime();

    }

    public int GetRank()
    {
        if (score > totalNote * 1.2 * 70)
        {
            return 5;
        }
        else if (score > totalNote * 0.8 * 70)
        {
            return 4;
        }
        else if (score > totalNote * 0.6 * 70)
        {
            return 3;
        }
        else if (score > totalNote * 0.4 * 70)
        {
            return 2;
        }
        else if (score > totalNote * 0.3 * 70)
        {
            return 1;
        }
        return 0;
    }


}
